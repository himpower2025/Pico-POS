...
const getAuthErrorMessage = (code?: string): string => {
  switch (code) {
    ...
    case 'auth/no-credential':
      return 'Sign-in did not return a valid credential. Please try again.';
...
const LoginView: React.FC<LoginViewProps> = ({ onLogin }) => {
  ...
  // Sign in with Apple is mandatory on iOS (Guideline 4.8) and available on
  // the web once the provider is configured. Hiding it on Android keeps the
  // sheet from offering a flow most Android users can't complete smoothly.
  const showAppleSignIn = Capacitor.getPlatform() !== 'android';

  /**
   * Load the store's profile by Auth UID, or create a blank one on first
   * login.
   *
   * The old version seeded fabricated business data — panNumber
   * '987-654-321', settlementAccount 'US-BANK-999', store name 'Pico Cafe'.
   * Those are tax and settlement fields that get PRINTED ON RECEIPTS, so a
   * real shop's first receipt carried a fake tax registration number until
   * someone noticed. New accounts now start genuinely empty; the Settings
   * screen prompts for the real values.
   *
   * The 'demo' convention is kept for App Review: a reviewer signing in with
   * an address containing "demo" gets a pre-populated sample store so they
   * see a working dashboard instead of an empty one.
   */
  const resolveOrCreateProfile = async (
    uid: string,
    ...
    if (!profile) {
      const isDemo = (userEmail || '').toLowerCase().includes('demo');
  ...
  const runAuth = async (fn: () => Promise<{ user: { uid: string; email: string | null } }>) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await runAuth(() =>
      mode === 'signup' ? registerWithEmail(email, password) : signInWithEmail(email, password)
    );
  };

  const handleGoogleLogin = () => runAuth(signInWithGoogleAccount);
  const handleAppleLogin = () => runAuth(signInWithAppleAccount);
        ...
        <div className="w-full md:w-1/2 p-8 md:p-12 flex flex-col justify-center bg-white">
          ...
          <div className="space-y-3">
            <button
              ...
              disabled={isLoading}
              className="w-full bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 font-bold py-3.5 rounded-xl shadow-sm transition-all flex items-center justify-center gap-3 disabled:opacity-70"
            >
            ...
            {/* Apple's Human Interface Guidelines require the official mark
                and a black or white button of equal visual weight to the
                other providers. Do not restyle this into a small text link —
                reviewers check. */}
            {showAppleSignIn && (
              <button
                type="button"
                ...
                disabled={isLoading}
                className="w-full bg-black hover:bg-gray-900 text-white font-bold py-3.5 rounded-xl shadow-sm transition-all flex items-center justify-center gap-3 disabled:opacity-70"
              >
